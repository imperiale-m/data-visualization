// Data
d3.csv("../data/data3.csv", d3.autoType).then(function (data) {
  console.log(data);

  const WIDTH3 = document.getElementById("task1").offsetWidth;
  const HEIGHT3 = window.innerHeight * 0.6;
  const l3 = Math.floor(WIDTH3 / 4);
  const r3 = 0; // Math.floor(WIDTH3 / 12);

  const margin3 = { top: 80, right: r3, bottom: 60, left: l3 };
  ...
  for (let i = 0; i < N; i++) {
    ...
    svg3
      .append("text")
      .attr("transform", `translate(${(width3 / 5) * i -30}, ${-30})`)
